...
const cors = (origin) => ({
  ...
  /* POST belongs here: /auth/login, /auth/logout and /auth/users are all
     POSTs from the browser, and a JSON body makes them preflighted. Without
     POST on this list the preflight succeeds and the real request is
     blocked, which surfaces as "couldn't reach the server" at sign-in. */
