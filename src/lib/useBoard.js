...
export function useBoard() {
  const [st, setSt] = useState({ orders: [], products: SEED, refundTypes: SEED_REFUND_TYPES, refunds: [], customers: [], blocks: [], wipes: [], calls: [], settings: DEF, updatedAt: 0 });
    ...
    try {
      ...
      if (r?.value) {
        ...
        if (!silent || (p.updatedAt || 0) > lu.current) {
          setSt({ products: SEED, refundTypes: SEED_REFUND_TYPES, refunds: [], customers: [], blocks: [], wipes: [], calls: [], settings: DEF, orders: [], ...p });
    ...
    try {
      ...
      if (typeof fn === "function") {
        ...
        if (remote && (remote.updatedAt || 0) > base) {
          toSave = fn({ products: SEED, refundTypes: SEED_REFUND_TYPES, refunds: [], customers: [], blocks: [], wipes: [], calls: [], settings: DEF, orders: [], ...remote });
